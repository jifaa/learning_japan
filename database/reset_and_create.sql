-- ============================================================
-- Learning Japan — RESET & CREATE FULL SCHEMA
-- ============================================================
-- One-shot: drops everything, then recreates.
-- Safe to run even if some objects are already missing.
-- ============================================================

-- 1. Drop functions CASCADE (auto-drops all dependent triggers)
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.update_updated_at_column() CASCADE;

-- 2. Drop ALL tables (order: dependents first)
...
DROP TABLE IF EXISTS public.kanji_characters CASCADE;
...
CREATE TYPE script_type AS ENUM ('hiragana','katakana','kanji','mixed','vocabulary','grammar');
CREATE TYPE srs_state AS ENUM ('new','learning','review','relearning','graduated');
CREATE TYPE srs_rating AS ENUM ('again','hard','good','easy');
CREATE TYPE quiz_type AS ENUM ('vocabulary','grammar','kanji','reading','mixed');
CREATE TYPE lesson_type AS ENUM ('vocabulary','grammar','kanji','reading','quiz','review');
CREATE TYPE achievement_category AS ENUM ('streak','progress','mastery','social','special');
...
CREATE INDEX idx_user_srs_cards_user_id ON public.user_srs_cards(user_id);
CREATE INDEX idx_user_srs_cards_due_date ON public.user_srs_cards(user_id, due_date);
CREATE INDEX idx_user_srs_cards_state ON public.user_srs_cards(user_id, state);
ALTER TABLE public.user_srs_cards ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage own SRS cards" ON public.user_srs_cards FOR ALL USING (auth.uid() = user_id);
...
CREATE INDEX idx_srs_review_log_user_id ON public.srs_review_log(user_id);
CREATE INDEX idx_srs_review_log_card_id ON public.srs_review_log(card_id);
ALTER TABLE public.srs_review_log ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage own review logs" ON public.srs_review_log FOR ALL USING (auth.uid() = user_id);
...
-- ============================================================
-- PUBLIC CONTENT TABLES (all 28 seed targets)
-- ============================================================
...
-- 8. radical_variants
CREATE TABLE public.radical_variants (
  id TEXT PRIMARY KEY,
...
-- 9. kanji_radical_map
CREATE TABLE public.kanji_radical_map (
  id TEXT PRIMARY KEY,
  jlpt_level TEXT,
  kanji TEXT NOT NULL,
...
-- 13. counter_forms
CREATE TABLE public.counter_forms (
  id TEXT PRIMARY KEY,
  jlpt_level TEXT,
...
-- 14. calendar_time
CREATE TABLE public.calendar_time (
  id TEXT PRIMARY KEY,
  jlpt_level TEXT,
...
CREATE TABLE public.quiz_answer_schemas (
  id TEXT PRIMARY KEY,
...
CREATE TABLE public.lesson_roadmap_tree (
  id TEXT PRIMARY KEY,
  jlpt_level TEXT,
  tree_version TEXT,
...
CREATE TABLE public.lesson_content_map (
  id TEXT PRIMARY KEY,
  lesson_id TEXT NOT NULL,
...
-- 24. srs_deck_configs
CREATE TABLE public.srs_deck_configs (
  id TEXT PRIMARY KEY,
  deck_key TEXT NOT NULL,
  display_name_id TEXT,
  description_id TEXT,
